...
#
# 用法：
#   bash run_local_pc2.sh                   # 完整流程
#   bash run_local_pc2.sh --force-calibrate # 強制重新校正（CQI mapping 有變時用）
#   bash run_local_pc2.sh --skip-calibrate  # 強制跳過 CQI 校正
#   bash run_local_pc2.sh --skip-baseline   # 跳過 PF Baseline 測試（已有數據時用）
#
# 流程（xApps 不會在 Baseline 測試期間干擾排程）：
#   Step 1: start_iab_client.sh --no-benchmark
#   Step 2: CQI 校正（首次或 --force-calibrate，已校正過自動跳過）
#   Step 3: PF Baseline 效能測試（xApps 尚未啟動，量到的是乾淨 OAI PF 數據）
#   Step 4: 通知 PC1 Baseline 完成 → PC1 才開始啟動 xApps
...
notify_pc1_baseline_done() {
    touch "$FLAG_BASELINE_DONE"
    if ssh $SSH_OPTS ${PC1_USER}@${PC1_IP} "touch /tmp/local_baseline_done" 2>/dev/null; then
        ok "已通知 PC1 Baseline 完成，PC1 將開始啟動 xApps"
    else
        warn "SSH 通知失敗，請在 PC1 手動執行：touch /tmp/local_baseline_done"
    fi
}

# ── 清除舊 flag ──────────────────────────────────────────────
rm -f "$FLAG_CALIBRATION_DONE" "$FLAG_BASELINE_DONE" "$FLAG_XAPPS_READY"
...
# 此時 PC1 的 xApps 尚未啟動，量到的是乾淨的 OAI PF Scheduler 基準數據
if [[ "$*" == *"--skip-baseline"* ]]; then
    warn "跳過 PF Baseline 測試（--skip-baseline）"
else
    log "Step 3: PF Baseline 效能測試（3 輪取平均）..."
    log "  （此時 xApps 尚未啟動，數據為乾淨 OAI PF Scheduler 基準）"
    bash "$COMPOSE_DIR/iab/iab_perf_test.sh" 3
    ok "PF Baseline 測試完成"
fi

# ── Step 4: 通知 PC1 Baseline 完成，PC1 才開始啟動 xApps ─────
log "Step 4: 通知 PC1 Baseline 完成..."
notify_pc1_baseline_done

# ── Step 5: 等待 PC1 xApps 就緒 ──────────────────────────────
log "Step 5: 等待 PC1 xApps 就緒..."
...
# ── Step 6: 啟動動態流量場景 ──────────────────────────────────
log "Step 6: 啟動動態流量場景（Scenario D，每相位 60 秒）..."
