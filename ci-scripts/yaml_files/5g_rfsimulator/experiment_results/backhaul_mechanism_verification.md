...
> **⚠️ 2026-09-13 更正：本文件第 1 節「MT 端 telnetsrv 命令正確回應」的結論是誤判，機制當時其實整段是靜默 no-op。**
> 事後查出 `bhload get` 命令從一開始就因為命名撞上 telnetsrv 框架保留字（`"get"`/`"set"` 被 `process_command()` 優先當成變數存取語法處理）而永遠回傳「未知命令」錯誤，`backhaul_prb_ratio` 全程 fail-safe 回退成 1.0（無約束），根本沒有真正縮小過任何節點的 PRB 池。當時只確認了「連線建立、telnetsrv log 印出 `Command received`」，沒有進一步檢查回傳內容是不是真的有效數據，才會誤判成功。第 2 節「UE5 吞吐量下降約 30%」這個因果效應觀察，文件本身已誠實記錄「無法排除是 PF 排程器本身同 cell 公平分配的效果」——現在確定完全就是這個原因，跟 backhaul-aware 機制無關（機制當時根本沒運作）。
> 真正的根因排查、修復過程，見 `HISTORY.md` 2026-09-13 條目；真正有效的機制生效驗證（`tcpdump` 直接抓包確認 `bhload query` 回傳真實數據）與重新量測的 Stage 1 基準，見 `PF.md`。本文件保留作為除錯過程的歷史記錄，不再代表機制的有效驗證。
