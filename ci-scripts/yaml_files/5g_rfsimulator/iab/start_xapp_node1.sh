#!/bin/bash
# =============================================================================
# start_xapp_node1.sh — Node 1 Local xApp 容器啟動腳本
#
# 職責：
...
XAPP_BIN="/node1_brain"
FLEXRIC_CONF="/usr/local/etc/flexric/flexric.conf"
PLUGIN_PATH="/usr/local/lib/flexric/"
RIC_IP="${NEAR_RT_RIC_IP:-192.168.88.141}"

echo "[start_xapp_node1] =========================================="
echo "[start_xapp_node1]  Node 1 Local xApp 啟動腳本"
echo "[start_xapp_node1]  FlexRIC Server IP: ${RIC_IP}"
echo "[start_xapp_node1] =========================================="

# -----------------------------------------------------------------------------
# 步驟 1：安裝 ZMQ / cJSON runtime 函式庫
# -----------------------------------------------------------------------------
echo "[start_xapp_node1] 安裝 runtime 相依套件..."
...
echo "[start_xapp_node1] 函式庫安裝完成"

# -----------------------------------------------------------------------------
# 步驟 2：等待 FlexRIC Server 主機可達
# -----------------------------------------------------------------------------
echo "[start_xapp_node1] 等待 FlexRIC Server (${RIC_IP}) 就緒..."
MAX_WAIT=120
ELAPSED=0

while ! ping -c 1 -W 2 "${RIC_IP}" >/dev/null 2>&1; do
    if [ ${ELAPSED} -ge ${MAX_WAIT} ]; then
        echo "[start_xapp_node1] WARNING: FlexRIC Server ${MAX_WAIT}s 內未就緒，仍嘗試啟動 xApp"
        break
    fi
    sleep 2
    ELAPSED=$((ELAPSED + 2))
    echo "[start_xapp_node1] 等待中... (${ELAPSED}/${MAX_WAIT}s)"
done
echo "[start_xapp_node1] FlexRIC Server 已就緒，啟動 xApp"

# -----------------------------------------------------------------------------
# 步驟 3：啟動 xApp
# -----------------------------------------------------------------------------
echo "[start_xapp_node1] 執行: ${XAPP_BIN} -c ${FLEXRIC_CONF} -p ${PLUGIN_PATH}"
