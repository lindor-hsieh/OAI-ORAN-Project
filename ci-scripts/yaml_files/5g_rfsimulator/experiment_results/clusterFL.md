...
**比較基準**：`PF.md`（Stage 1，JFI=0.3303、平均吞吐量 6.45 Mbps、平均 RTT 224.23 ms，seed `20260914`）；`avgFL.md`（Stage 2）的有效重測數據尚在補做（見該文件最上方更正說明），本文件暫時只跟 PF baseline 做三方比較中的兩方，avg FL 那一欄等 Stage 2 重測完成後補上

> ## ⚠️ 本次量測有兩個已知confound，數字如實記錄，但暫不建議直接當作「cluster FL 演算法本身好壞」的定論
>
> 1. **量測前才發現並修復一個活躍中的 `REWARD_MODE` 傳遞 bug**：全部 12 個 `flower-supernode-nodeN` 容器整個 Stage 3 訓練期間（含這次量測當下）都跑在 `REWARD_MODE=lagrangian`，不是預期的 `throughput_only`（`run_stage2_fl.sh` 與 `training_watchdog.sh::full_recovery()` 帶起這批服務時都沒有明確傳遞 `REWARD_MODE`，只帶了 `FL_MODE`/`MODEL_ARCH`，導致 docker-compose 的 `${REWARD_MODE:-lagrangian}` 預設值生效）。追查程式碼後確認：`drl_agent.py` 的 `self._lambda`（Lagrangian 乘子）只有在記錄 metrics／存讀 checkpoint 時被使用，**MLP 訓練路徑的 actor/critic loss 完全沒有用到它**，且 FL 訓練路徑（`training_pipeline.py`／`client_app.py`）純粹讀取 MongoDB 裡已經算好的 `reward` 欄位（由 `inference-nodeN` 用正確的 `throughput_only` 公式寫入），不會重新計算——所以這個 bug **不會**直接污染 policy 權重的梯度更新，但仍違反了「Stage 2~4 全程應該是 `throughput_only`」的既有設計不變式，且代表 checkpoint 裡的 `lambda` 欄位是髒的。已在量測前修好（腳本＋現場強制重建 12 個 `flower-supernode-nodeN`），詳細根因與修復見 `HISTORY.md` 2026-09-19 條目。
> 2. **整晚（約 8.5 小時）的訓練期間歷經 11 次 FlexRIC 崩潰自動/半自動復原**，過程中每個節點的 UE 連通性都反覆出現過「tunnel 自發性重建、DNAT/路由過期、DU 建立被跳過」這類已知的基礎設施不穩定模式（見 `HISTORY.md` 同日多個條目），量測本身執行的 15 分鐘窗口內三主機 `RestartCount` 雖然全程沒有新增崩潰，但無法排除個別 UE 在量測窗口內殘留連通性不佳（例如本次多個 UE 的吞吐量覆蓋率明顯偏低，見下方結果表），這會拉低量到的平均值，跟 policy 本身的決策品質混在一起，難以完全分離。
>
> 兩點合起來，這份量測比較適合定位成「這次乾淨重啟＋長時間訓練後的一次誠實快照」，不是「cluster FL 演算法優劣的最終結論」——如果要對 cluster FL 下定論，建議在上面兩個 confound 都排除的乾淨環境下（`REWARD_MODE` bug 已修好；建議再確認一次量測窗口全程 17/17 UE 覆蓋率都接近 100%）重新量一次。
>
> **2026-09-19 更新：已依上述建議重新量測一次，下方「量測結果」已換成這次的乾淨數字**——量測開始前明確逐項驗證：17/17 UE 現場 ping 確認 0% 封包遺失（含 UE17）、`docker ps` 確認 12/12 xApp 容器存在、`xapp-node1` log 確認有真實的 `CONTROL-REQUEST tx`/`CONTROL ACK rx`/`AI 決策` 訊號（不是只看容器數量）、`FL_MODE=cluster`／`REWARD_MODE=throughput_only`／`MODEL_ARCH=mlp` 三個環境變數逐一確認。confound 1（`REWARD_MODE` bug）在這次量測前已修復並生效；confound 2（連通性殘留）這次透過量測前的完整驗證排除——量測全程 17 個 UE 都量到非零吞吐量（0/17 零樣本），比舊版本（1/17 零樣本）更乾淨。這次的數字可以視為目前对 cluster FL 較可信的量測結果，舊版本數字保留在下方供對照但不建議引用。
...
- **穩定性事件**：期間共發生 11 次 FlexRIC 崩潰，前 2 次因為 `training_watchdog.sh` 當時的既有設計缺陷（單一主機軟性失敗就整個中止復原，導致 xApp 層被晾著）需要人工介入排除，修復該缺陷後第 3~10 次都全自動、無人工介入復原成功；第 11 次因為在 600 秒冷卻窗內又復發，`training_watchdog.sh` 依設計正確判斷「復原沒生效」、自行停止等待人工介入（見 `HISTORY.md`），本次量測前的乾淨重啟即是針對這次事件的處理。
...
4. **`rfsim5g-donor-cu` RestartCount 基準點確認為 1**（沿用當時基準，量測全程未再增加）。
...
3. **吞吐量/RTT 取樣覆蓋率偏低是這次量測的主要特徵**（多數 UE 吞吐量覆蓋率在 30~65% 之間、RTT 覆蓋率普遍個位數到低雙位數百分比），但這次全部 17 個 UE 都至少量到部分非零樣本，不是「完全連不通」——比較像是在 Scenario R 高負載場景下，backhaul-aware PRB 預算機制與/或目前的 cluster FL policy 讓多數節點只能間歇性搶到排程機會，這是policy 決策品質或機制本身的特性，不是連通性故障，值得後續深入分析（例如逐 UE 檢視是否某些角色比例 `role_ratio_i` 的節點特別容易被排擠）。
4. **與 PF.md 的既有觀察一致的部分**：UE1~4（PC2 access 節點）覆蓋率偏低（25~39%）、深層跨主機節點（PC3 UE9~16）平均吞吐量普遍偏低（0.46~1.49 Mbps）與既有模式相符；UE5~8（PC1）表現相對最好（13.6~23.8 Mbps），跟 PF/舊版本的既有模式一致。
...
訓練並未因這次量測而停止或重置——量測用的是同一份持續累積的 MongoDB 經驗與 checkpoint。量測結束後會暫時封存這份 Stage 3 資料，換回 Stage 2 的封存 checkpoint 做 Stage 2 的有效重測（`avgFL.md` 目前的量測結果因為 xApp 未啟動而作廢，這次補做），完成後再把 Stage 3 的資料還原回來、以修好 `REWARD_MODE` bug 後的乾淨設定繼續訓練。
...
**量測前逐項驗證**：17/17 UE 現場 ping 確認 0% 封包遺失（含 UE17）；`docker ps` 確認 12/12 xApp 容器皆為 running；`rfsim5g-oai-ext-dn` 確認 17 個獨立 iperf3 server port（5201~5217）皆在監聽；Scenario T、`measure_stage.py`，15 分鐘（900s）、每 5 秒取樣一次；量測過程中三主機 `RestartCount` 全程無新增崩潰。**上線過程額外排查並修復兩次 FlexRIC 內部 xApp-association 狀態卡死**（`xapp-node1` 停在 `Resending Setup Request after timeout`、`flexric` log 顯示大量 `RIC Indication...but no xApp associated`，即使 DU 端 E2AP 連線正常）——單獨重啟 xApp 不夠，需要 FlexRIC→全部 DU→xApp 依序重啟才解決；過程中也再次遇到 `rfsim5g-iab-du-5` 靜默未啟動的已知缺口，詳見 `HISTORY.md` 對應條目。
...
2. **這次已經排除了先前懷疑的主要 confound**（訓練/測試 scenario 曝光比例不對稱），改用 Scenario T 做這次比較，Stage 2 也已用相近輪替表重新訓練過。即使如此，cluster FL 仍未展現優於 PF 或 avg FL 的效果，代表先前假設的「訓練/測試場景不對稱」並非 cluster FL 表現不佳的主要或唯一原因，需要回頭檢視聚合演算法本身（`role_ratio_i` 加權機制）或 local DRL 在 cluster FL 聚合下的收斂特性。
