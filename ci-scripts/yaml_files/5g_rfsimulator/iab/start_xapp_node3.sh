...
echo "[start_xapp_node3]  Node 3 Local xApp 啟動腳本"
echo "[start_xapp_node3]  FlexRIC Server IP: ${RIC_IP}"
echo "[start_xapp_node3] =========================================="
...
#   (最多等 120 秒，每 2 秒探測一次)
# -----------------------------------------------------------------------------
echo "[start_xapp_node3] 等待 FlexRIC Server (${RIC_IP}:36421) 就緒..."
MAX_WAIT=120
ELAPSED=0
...
while ! ping -c 1 -W 2 "${RIC_IP}" >/dev/null 2>&1; do
    ...
    sleep 2
    ELAPSED=$((ELAPSED + 2))
    echo "[start_xapp_node3] 等待中... (${ELAPSED}/${MAX_WAIT}s)"
done
