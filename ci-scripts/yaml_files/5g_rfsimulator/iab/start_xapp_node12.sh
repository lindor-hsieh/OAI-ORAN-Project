#!/bin/bash
# =============================================================================
# start_xapp_node12.sh — Node 12 Local xApp 容器啟動腳本
#
# 職責：
#   2. 等待 FlexRIC Server 就緒 (透過 NEAR_RT_RIC_IP 環境變數)
#   3. 啟動 xapp_node12 執行檔
#
...
XAPP_BIN="/node12_brain"
FLEXRIC_CONF="/usr/local/etc/flexric/flexric.conf"
PLUGIN_PATH="/usr/local/lib/flexric/"
RIC_IP="${NEAR_RT_RIC_IP:-192.168.88.141}"

echo "[start_xapp_node12] =========================================="
echo "[start_xapp_node12]  Node 12 Local xApp 啟動腳本"
echo "[start_xapp_node12]  FlexRIC Server IP: ${RIC_IP}"
echo "[start_xapp_node12] =========================================="
...
#   (最多等 120 秒，每 2 秒探測一次)
# -----------------------------------------------------------------------------
echo "[start_xapp_node12] 等待 FlexRIC Server (${RIC_IP}:36421) 就緒..."
MAX_WAIT=120
ELAPSED=0
...
while ! ping -c 1 -W 2 "${RIC_IP}" >/dev/null 2>&1; do
    if [ ${ELAPSED} -ge ${MAX_WAIT} ]; then
        echo "[start_xapp_node12] WARNING: FlexRIC Server ${MAX_WAIT}s 內未就緒，仍嘗試啟動 xApp"
        break
    fi
    sleep 2
    ELAPSED=$((ELAPSED + 2))
    echo "[start_xapp_node12] 等待中... (${ELAPSED}/${MAX_WAIT}s)"
done
echo "[start_xapp_node12] FlexRIC Server 已就緒，啟動 xApp"

# -----------------------------------------------------------------------------
# 步驟 2：啟動 xApp
#   exec 取代目前 shell，讓 PID 1 是 xApp 本身，確保 SIGTERM 能正常傳遞。
# -----------------------------------------------------------------------------
echo "[start_xapp_node12] 執行: ${XAPP_BIN} -c ${FLEXRIC_CONF} -p ${PLUGIN_PATH}"
